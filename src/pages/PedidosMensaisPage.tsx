...
export function PedidosMensaisPage() {
  ...
  // Copia a imagem PNG do fechamento — mesmo layout do PDF, desenhado direto no Canvas 2D
  async function handleCopiarPNG(c: Cliente, pedidosCliente: PedidoCliente[], pesoTotal: number, valorTotal: number) {
    try {
      ...
      const DPI   = 2          // fator de escala para alta resolução
      const W     = 794 * DPI  // 210mm em px@96dpi × escala
      const H     = 1123 * DPI // 297mm em px@96dpi × escala
      const MM    = W / 210    // px por mm
      ...
      // Helpers
      const mm = (v: number) => v * MM / DPI  // mm → px (já escalado)
      ...
      const titleW = ctx.measureText('Fechamento de Mensalista').width
      ctx.fillText('Fechamento de Mensalista', mr - titleW, cy + mm(6))
      ctx.fillStyle = GRAY
      ...
      roundRect(ctx, ml, cy, cw, mm(20), mm(2))
      ctx.fill(); ctx.stroke()
      ...
      ctx.fillStyle = GRAY; ctx.font = `${mm(2.8)}px Arial, sans-serif`
      ctx.fillText(`Plano Mensal · Pgto: ${FORMA_PAGTO_LABELS[c.forma_pagamento] || c.forma_pagamento}`, hx, cy + mm(16))
      cy += mm(26)
      ...
      ;['DATA', 'PEÇAS LAVADAS', 'PESO', 'VALOR'].forEach((h, i) => {
        ...
        if (i >= 2) {
          const tw2 = ctx.measureText(h).width
          ctx.fillText(h, xs[i] + ws[i] - tw2 - mm(2), cy + mm(4.8))
        } else {
      ...
      ;[...pedidosCliente].reverse().forEach((p, idx) => {
        ...
        ctx.font = `${mm(2.8)}px Arial, sans-serif`
        const pecasLines = wrapText(ctx, pecas, colWmm[1] - mm(4))
      ...
      roundRect(ctx, ml, cy, cw, mm(16), mm(2)); ctx.fill(); ctx.stroke()
      ...
      totItems2.forEach((t) => {
        ...
        ctx.fillText(t.label, t.cx - lw / 2, cy + mm(5))
        ...
        ctx.fillText(t.value, t.cx - vw / 2, cy + mm(12))
      })
      ...
      roundRect(ctx, ml, cy, cw, mm(22), mm(2)); ctx.fill(); ctx.stroke()
      ...
      ctx.fillText(`Chave PIX (CNPJ): ${chavePix || '59.815.300/0001-71'}`, ml + mm(4), cy + mm(19))
      cy += mm(28)
      ...
      ctx.fillText(footerText, (794 - fw * DPI / DPI) / 2, cy)
  ...
  async function handleCompartilharPDF(c: Cliente, pedidosCliente: PedidoCliente[], pesoTotal: number, valorTotal: number) {
    try {
      ...
      const CW = PW - ML - MR   // largura do conteúdo: 182mm
      ...
      // Helper: texto centralizado em X
      ...
      // ── LOGO + CABEÇALHO ────────────────────────────────
      try {
        pdf.addImage(logoBase64, 'PNG', ML, y, 14, 14)
      ...
      const titleText = 'Fechamento de Mensalista'
      const titleW = pdf.getTextWidth(titleText)
      pdf.text(titleText, PW - MR - titleW, y + 6)
      ...
      pdf.setFontSize(8)
      pdf.setTextColor(...GRAY)
      pdf.text(`Plano Mensal · Pgto: ${FORMA_PAGTO_LABELS[c.forma_pagamento] || c.forma_pagamento}`, halfX, y + 16)

      y += 26

      // ── TABELA DE PEDIDOS ────────────────────────────────
      const colW = [26, CW - 26 - 22 - 28, 22, 28] // Data | Peças | Peso | Valor
      ...
      pedidosOrdenados.forEach((p, idx) => {
        ...
        // Calcular altura necessária (pode ser multi-linha)
      ...
      const totItems = [
        { label: 'QUANTIDADE DE ENVIOS', value: String(pedidosCliente.length), cx: ML + CW * 0.165, align: 'center' as const },
        { label: 'PESO TOTAL', value: `${Number(pesoTotal).toLocaleString('pt-BR')} kg`, cx: (tot1X + tot2X) / 2, align: 'center' as const },
        { label: 'TOTAL A PAGAR', value: formatBRL(valorTotal), cx: (tot2X + tot3X) / 2, align: 'center' as const },
      ]
      ...
      pdf.setFontSize(8.5)
      pdf.setTextColor(...BLUE)
      pdf.text(`Chave PIX (CNPJ): ${chavePix || '59.815.300/0001-71'}`, ML + 4, y + 19)
