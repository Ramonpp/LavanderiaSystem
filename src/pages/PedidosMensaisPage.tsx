...
export function PedidosMensaisPage() {
  ...
  async function handleCopiarPNG(c: Cliente, pedidosCliente: PedidoCliente[], pesoTotal: number, valorTotal: number) {
    try {
      ...
      const tw = pdf.getTextWidth('Fechamento de Mensalista')
      pdf.text('Fechamento de Mensalista', PW - MR - tw, y + 6)
      ...
      pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8); pdf.setTextColor(...GRAY)
      pdf.text(`Plano Mensal · Pgto: ${FORMA_PAGTO_LABELS[c.forma_pagamento] || c.forma_pagamento}`, hx, y + 16)
      y += 26
